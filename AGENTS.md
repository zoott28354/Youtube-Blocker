# YouTube Blocker — Contesto per Codex
...
| `src/hooks/useBlocker.ts` | Stato React centralizzato, invoke Tauri, status arricchito con liste attive |
| `src/components/BlockLists.tsx` | Preset, liste custom, toggle, edit, duplica, varianti dominio modificabili |
| `src/components/StatusCard.tsx` | Stato blocco, indicatori hosts/firewall/policy, liste attive |
| `src/components/About.tsx` | Tab info, versione app dal backend, link GitHub |
...
- In edit mode frontend, le varianti restano visibili e modificabili singolarmente
...
get_sites()              → Vec<String>   // siti delle sole liste attive
get_lists()              → Vec<BlockList>
create_list(name)        → Result<BlockList>
update_list(id, name, sites)
delete_list(id)
toggle_list(id, active)
add_site_to_list(list_id, domain)        // espande automaticamente www/m varianti
remove_site_from_list(list_id, domain)   // rimuove root + varianti
block_all()              → Result<()>    // no PIN, richiede almeno una lista attiva
unblock_all(pin)         → Result<()>    // PIN obbligatorio, spegne tutte le liste
has_pin()                → bool
set_pin(pin)             → Result<()>
...
## Preferenze utente
- NON includere "Co-Authored-By: Codex Sonnet 4.6" nei commit
